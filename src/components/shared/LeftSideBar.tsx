...
const LeftSideBar = () => {
    ...
    const {mutateAsync: signOut, isPending} = useSignOutAccountMutation();

    const logOutController = async () => {
        await signOut();

        localStorage.setItem("cookieFallback", "[]");
        toast({title: "Successfully Logout"})
    ...
    return (
        <nav className="leftsidebar">
            ...
            <Button
                ...
                <span className="small-medium lg:base-medium ! font-bold">
					{!isPending ? "Log Out" : "Logging Out"}
				</span>
            </Button>
        </nav>
    );
